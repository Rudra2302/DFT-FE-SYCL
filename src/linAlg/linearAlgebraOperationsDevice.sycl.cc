...
namespace dftfe
{
  namespace linearAlgebraOperationsDevice
  {
    ...
    {
      void
      combinedDeviceKernel(sycl::nd_item<1>   ind,
        ...
        for (unsigned int index = globalThreadId; index < numberEntries;
             index += n_workgroups * n_workitems)
          {
            ...
            *(Y + index)            = dftfe::utils::makeComplex(
              (Y + index)->real() * (*(sqrtMassVec + blockIndex) * 1.0 / scalarOld),
              (Y + index)->imag() * (*(sqrtMassVec + blockIndex) * 1.0 / scalarOld));
            *(X + index) = dftfe::utils::makeComplex(
              (X + index)->real() * (*(invSqrtMassVec + blockIndex)),
              (X + index)->imag() * (*(invSqrtMassVec + blockIndex)));
            Y[index] =
              dftfe::utils::makeComplex(a * X[index].real() + b * Y[index].real(),
                                        a * X[index].imag() + b * Y[index].imag());
            *(X + index) = dftfe::utils::makeComplex(
              (X + index)->real() * (*(invSqrtMassVec + blockIndex) * scalar),
              (X + index)->imag() * (*(invSqrtMassVec + blockIndex) * scalar));
            *(Y + index) = dftfe::utils::makeComplex(
              (Y + index)->real() * (*(sqrtMassVec + blockIndex)),
              (Y + index)->imag() * (*(sqrtMassVec + blockIndex)));
          }
      }
      ...
      void
      addSubspaceRotatedBlockToXKernel(sycl::nd_item<1>   ind,
      ...
      void
      copyFromOverlapMatBlockToDPSPBlocks(sycl::nd_item<1>   ind,
      ...
      void
      computeDiagQTimesXKernel(sycl::nd_item<1>   ind,
      ...
      // R^2=||Y-X*Gamma||^2
      void
      computeResidualDeviceKernel(sycl::nd_item<1>   ind,
                                  ...
                                  double *           r)
      {
        ...
        for (int i = ind.get_global_id(0); i < numVectors * numDofs;
             i += n_workgroups * n_workitems)
          {
             ...
             i += n_workgroups * n_workitems)
          {
            ...
            const dftfe::utils::deviceDoubleComplex diff =
              dftfe::utils::makeComplex(
                Y[i].real() - X[dofIndex * N + startingVecId + waveIndex].real() *
                           eigenValues[startingVecId + waveIndex],
                Y[i].imag() - X[dofIndex * N + startingVecId + waveIndex].imag() *
                           eigenValues[startingVecId + waveIndex]);
            r[i] = diff.real() * diff.real() + diff.imag() * diff.imag();
          }
      }
      ...
      const bool                                       rotationMatTranspose)
    {
      ...
      dftfe::utils::deviceMemcpyH2D(
        ...
        size_t total_workitems = ((M * N + (dftfe::utils::DEVICE_BLOCK_SIZE - 1)) /
                                   dftfe::utils::DEVICE_BLOCK_SIZE) * dftfe::utils::DEVICE_BLOCK_SIZE;
        auto diagValues_begin = diagValues.begin();
        dftfe::utils::deviceEvent_t event = streamCompute.parallel_for(
                                    sycl::nd_range<1>(total_workitems,dftfe::utils::DEVICE_BLOCK_SIZE), 
                                    [=](sycl::nd_item<1> ind){
            computeDiagQTimesXKernel(ind, diagValues_begin, X, N, M);
        });
                  ...
                  if (BDof != 0)
                    {
                      dftfe::utils::deviceBlasWrapper::gemm(
                        ...
                        BVec);
#ifdef DFTFE_WITH_DEVICE_LANG_SYCL
                      size_t total_workitems = ((BVec * BDof + (dftfe::utils::DEVICE_BLOCK_SIZE - 1)) /
                                                  dftfe::utils::DEVICE_BLOCK_SIZE) * dftfe::utils::DEVICE_BLOCK_SIZE;
                      auto rotatedVectorsMatBlockSP_begin = rotatedVectorsMatBlockSP.begin();
                      dftfe::utils::deviceEvent_t event = streamCompute.parallel_for(
                                                  sycl::nd_range<1>(total_workitems,dftfe::utils::DEVICE_BLOCK_SIZE), 
                                                  [=](sycl::nd_item<1> ind){
                          addSubspaceRotatedBlockToXKernel(ind, BDof, BVec, rotatedVectorsMatBlockSP_begin,
                                                            X, idof, jvec, N);
                      });
                      DEVICE_API_CHECK(event);
#endif
      ...
      const dftParameters &                            dftParams)
    {
      ...
      dftfe::utils::deviceStream_t streamDeviceCCL{sycl::gpu_selector_v};
      ...
      for (unsigned int ivec = 0; ivec < N; ivec += vectorsBlockSize)
        {
          ...
          if ((ivec + B) <=
              ...
              (ivec + B) > bandGroupLowHighPlusOneIndices[2 * bandGroupTaskId])
            {
              ...
              const unsigned int DRem = D - B;
// #ifdef DFTFE_WITH_DEVICE_LANG_SYCL
              // size_t total_workitems = ((D * B + (dftfe::utils::DEVICE_BLOCK_SIZE - 1)) /
              //                             dftfe::utils::DEVICE_BLOCK_SIZE) * dftfe::utils::DEVICE_BLOCK_SIZE;
              // dftfe::utils::deviceEvent_t event = streamDataMove.parallel_for(
              //                             sycl::nd_range<1>(total_workitems,dftfe::utils::DEVICE_BLOCK_SIZE), 
              //                             [=](sycl::nd_item<1> ind){
              //     copyFromOverlapMatBlockToDPSPBlocks(ind, B, D, overlapMatrixBlock.begin(),
              //                                          overlapMatrixBlockDP.begin(), overlapMatrixBlockSP.begin());
              // });
              // DEVICE_API_CHECK(event);
// #endif
...
// #ifdef DFTFE_WITH_DEVICE_LANG_SYCL
//               size_t total_workitems = ((B + (dftfe::utils::DEVICE_BLOCK_SIZE - 1)) /
//                                           dftfe::utils::DEVICE_BLOCK_SIZE * M) * dftfe::utils::DEVICE_BLOCK_SIZE;
//               auto eigenValuesDevice_begin = eigenValuesDevice.begin();
//               auto HXBlockFull_begin = HXBlockFull.begin();
//               auto residualSqDevice_begin = residualSqDevice.begin();
//               dftfe::utils::deviceEvent_t event = streamDataMove.parallel_for(
//                                           sycl::nd_range<1>(total_workitems,dftfe::utils::DEVICE_BLOCK_SIZE), 
//                                           [=](sycl::nd_item<1> ind){
//                   computeResidualDeviceKernel(ind, B, M, N, jvec, eigenValuesDevice_begin, X,
//                                                 HXBlockFull_begin, residualSqDevice_begin);
//               });
//               DEVICE_API_CHECK(event);
// #endif
