...
namespace dftfe
{
  ...
  void
  applyPreconditionAndComputeDotProductKernel(sycl::nd_item<1> ind,
                                              Type *           d_dvec,
                                              Type *           d_devSum,
                                              const Type *     d_rvec,
                                              const Type *     d_jacobi,
                                              const int        N)
  {
//     Type* smem = sycl::malloc_shared<Type>(blockSize,dftfe::utils::deviceBlasWrapper::d_streamId);

//     int tid = ind.get_local_id(0);
//     int idx = ind.get_local_id(0) + ind.get_group(0) * (blockSize * 2);
...
//     smem[tid] = localSum;
//     // dftfe::utils::deviceBlasWrapper::d_streamId.wait();
...
//     if (tid == 0){
//       auto atomic_add = sycl::atomic_ref<Type, sycl::memory_order::relaxed,
...
//     }
  ...
  template <typename Type, int blockSize>
  void
  applyPreconditionComputeDotProductAndSaddKernel(sycl::nd_item<1> ind,
                                                  Type *           d_qvec,
                                                  Type *           d_devSum,
                                                  const Type *     d_rvec,
                                                  const Type *     d_jacobi,
...
//     }
  ...
  template <typename Type, int blockSize>
  void
  scaleXRandComputeNormKernel(sycl::nd_item<1> ind,
                              Type *           x,
                              Type *           d_rvec,
                              Type *           d_devSum,
                              const Type *     d_qvec,
    ...
    dftfe::utils::deviceMemset(d_devSumPtr, 0, sizeof(double));
...
#ifdef DFTFE_WITH_DEVICE_LANG_SYCL
        size_t total_workitems = blocks * dftfe::utils::DEVICE_BLOCK_SIZE;
        auto d_dvec_begin = d_dvec.begin();
        auto d_rvec_begin = d_rvec.begin();
        auto d_devSumPtr_begin = d_devSumPtr;
        auto d_jacobi_begin = d_jacobi;
        auto d_xLocalDof_begin = d_xLocalDof;
        ...
        dftfe::utils::deviceEvent_t event = stream.parallel_for(
            ...
            applyPreconditionAndComputeDotProductKernel<double,
                                                ...
                                                (ind, d_dvec_begin, d_devSumPtr_begin, d_rvec_begin, d_jacobi_begin, d_xLocalDof_begin);
        });
    ...
    const double *d_jacobi)
  {
    ...
    dftfe::utils::deviceMemset(d_devSumPtr, 0, sizeof(double));
...
#ifdef DFTFE_WITH_DEVICE_LANG_SYCL
        size_t total_workitems = blocks * dftfe::utils::DEVICE_BLOCK_SIZE;
        auto d_qvec_begin = d_qvec.begin();
        auto d_rvec_begin = d_rvec.begin();
        auto d_devSumPtr_begin = d_devSumPtr;
        auto d_jacobi_begin = d_jacobi;
        auto d_xLocalDof_begin = d_xLocalDof;
        ...
        dftfe::utils::deviceEvent_t event = stream.parallel_for(
                                            sycl::nd_range<1>(total_workitems,dftfe::utils::DEVICE_BLOCK_SIZE), 
                                            [=](sycl::nd_item<1> ind){
            applyPreconditionComputeDotProductAndSaddKernel<double, dftfe::utils::DEVICE_BLOCK_SIZE>
                                            (ind, d_qvec_begin, d_devSumPtr_begin, d_rvec_begin, d_jacobi_begin, d_xLocalDof_begin);
        });
        DEVICE_API_CHECK(event);
#endif
