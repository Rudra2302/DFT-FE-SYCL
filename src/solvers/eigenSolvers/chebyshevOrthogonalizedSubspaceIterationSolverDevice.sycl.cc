...
namespace dftfe
{
  namespace
  {
                  ...
                  const unsigned int startingXVecId)
    {
      ...
      const unsigned int numGangsPerBVec = (BVec + ind.get_local_range(0) - 1) / ind.get_local_range(0);
      const unsigned int gangBlockId     = ind.get_group(0) / numGangsPerBVec;
      const unsigned int localThreadId =
        globalThreadId - gangBlockId * numGangsPerBVec * ind.get_local_range(0);

      if (globalThreadId < M * numGangsPerBVec * ind.get_local_range(0) &&
          localThreadId < BVec)
        {
         ...
         jvec += numSimultaneousBlocksCurrent * vectorsBlockSize)
      {
        ...
        if ((jvec + numSimultaneousBlocksCurrent * BVec) <=
        ...
        else
          {
            ...
            size_t total_workitems = ((numSimultaneousBlocksCurrent * BVec +
    ...
    if (numberBandGroups > 1)
      {
        ...
        dftfe::utils::deviceMemcpyD2H(&eigenVectorsFlattened[0],
          eigenVectorsFlattenedDevice,
  ...
  {
    dftfe::utils::deviceBlasHandle_t deviceBlasHandle =
      BLASWrapperPtr->getDeviceStream();
