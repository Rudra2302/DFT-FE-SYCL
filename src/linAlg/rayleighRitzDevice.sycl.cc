    ...
    namespace
    {
                    ...
                    const unsigned int startingXVecId)
      {
        ...
        const unsigned int numGangsPerBVec =
          (BVec + ind.get_group(0) - 1) / ind.get_group(0);
        const unsigned int gangBlockId = ind.get_group(0) / numGangsPerBVec;
        const unsigned int localThreadId =
          globalThreadId - gangBlockId * numGangsPerBVec * ind.get_group(0);

        if (globalThreadId < M * numGangsPerBVec * ind.get_group(0) &&
            localThreadId < BVec)
          {
                    ...
                    const unsigned int                 startingXVecId)
      {
        ...
        if (globalThreadId < M * numGangsPerBVec * ind.get_group(0) &&
            localThreadId < BVec)
          {
            *(yVec + gangBlockId * N + startingXVecId + localThreadId) = std::complex<double>(0.0, 0.0);
          }
      }
    } // namespace
